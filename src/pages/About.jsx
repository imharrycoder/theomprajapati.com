...
// Helper to map tech stack string to an icon
function getTechIcon(name) {
  ...
  if (n.includes('css') || n.includes('tailwind')) return n.includes('tailwind') ? <SiTailwindcss /> : <FaCss3Alt />;
  if (n.includes('java') || n.includes('js')) return <SiJavascript />;
  ...
  if (n.includes('host')) return <SiHostinger />;
  if (n.includes('net')) return <SiNetlify />;
  if (n.includes('ver')) return <SiVercel />;
  if (n.includes('cloud')) return <SiGooglecloud />;
    ...
    <>
      ...
      {aboutPage.hero?.enabled !== false && (
        <section className="section-band border-b border-[var(--line)] pt-32 pb-20 relative overflow-hidden">
          ...
          <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-[var(--accent-2)] opacity-10 blur-[150px] rounded-full pointer-events-none pulse-neon" />
          <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-[var(--accent)] opacity-10 blur-[120px] rounded-full pointer-events-none" />
          
          <div className="shell relative z-10 grid gap-16 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div data-aos="fade-right">
              <span className="tag mb-8 inline-flex items-center gap-2 border-[var(--accent-2)] text-[var(--accent-2)] shadow-[0_0_15px_rgba(59,130,246,0.2)] scan-line overflow-hidden">
            ...
            <div className="relative items-start" data-aos="fade-left">
              <div className="neon-card neon-glow-cyan p-8 sm:p-10">
                <div className="absolute top-0 right-0 p-4 opacity-20">
                  <LayoutTemplate size={120} className="text-[var(--accent-2)]" />
                </div>
                <h2 className="text-2xl font-black text-[var(--text)] flex items-center gap-3 mb-8 relative z-10">
                  <span className="w-3 h-3 rounded-full bg-[var(--accent-2)] shadow-[0_0_10px_var(--accent-2)] animate-pulse" />
                ...
                <div className="grid gap-4 relative z-10">
                  {(aboutPage.services?.items || []).slice(0, 6).map((item, idx) => (
                    ...
                    >
                      <CheckCircle2 className="shrink-0 text-[var(--accent-2)] group-hover:drop-shadow-[0_0_8px_var(--accent-2)] transition-all" size={22} />
      ...
      {(aboutPage.vision?.enabled !== false || aboutPage.mission?.enabled !== false) && (
        <section className="section-band relative">
          ...
          <div className="shell grid gap-10 md:grid-cols-2 items-start relative z-10">
            {aboutPage.vision?.enabled !== false && (
              <article className="neon-card neon-glow-purple p-10 group" data-aos="fade-up">
                ...
                <h2 className="text-sm font-black uppercase tracking-[0.3em] text-[var(--accent)] mb-6 flex items-center gap-2">
                ...
                <div className="grid gap-6">
                  {(aboutPage.vision.principles || []).map((p, i) => (
                    <div key={p.title} className="glass p-5 rounded-xl border border-[var(--line)] hover:border-[var(--accent)]/50 transition-colors">
                      <h4 className="text-lg font-black text-[var(--text)] mb-2 flex items-center gap-3">
                        <span className="text-[var(--accent)] opacity-50 text-sm">0{i + 1}</span>
                        {p.title}
                      </h4>
            ...
            {aboutPage.mission?.enabled !== false && (
              <article className="neon-card neon-glow-cyan p-10 group flex flex-col" data-aos="fade-up" data-aos-delay="100">
                <h2 className="text-sm font-black uppercase tracking-[0.3em] text-[var(--accent-2)] mb-6 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-[var(--accent-2)] shadow-[0_0_10px_var(--accent-2)] animate-pulse" />
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      {(aboutPage.whyWorkWithMe.items || []).map(item => (
                        <div key={item} className="flex items-start gap-3 group/item">
                          <CheckCircle2 className="shrink-0 text-[var(--accent-2)] mt-0.5 opacity-70 group-hover/item:opacity-100 group-hover/item:drop-shadow-[0_0_8px_var(--accent-2)] transition-all" size={20} />
          ...
          <div className="shell">
            ...
            <div className="mt-20 grid gap-16 lg:grid-cols-2 items-start">
              ...
              {aboutPage.experience?.enabled !== false && (
                <div>
                  <h3 className="text-3xl font-black text-[var(--text)] flex items-center gap-4 mb-10">
                    <span className="p-3 bg-[var(--surface-3)] rounded-xl text-[var(--accent)] shadow-[0_0_20px_rgba(245,158,11,0.15)]">
                      <Briefcase size={28} />
                    </span>
                    {aboutPage.experience.title}
                  </h3>
                  <div className="space-y-8 relative before:absolute before:inset-0 before:ml-[22px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-[var(--accent)] before:via-[var(--accent-2)] before:to-transparent">
                    {(aboutPage.experience.items || []).map((exp, i) => (
                      <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active" data-aos="fade-up" data-aos-delay={i * 100}>
                        <div className="flex items-center justify-center w-12 h-12 rounded-full border-4 border-[var(--bg)] bg-[var(--accent)] text-white shadow-[0_0_15px_rgba(245,158,11,0.5)] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 transition-transform group-hover:scale-110">
                          <Briefcase size={18} />
                        </div>
                        <div className="neon-card neon-glow-purple p-6 w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] hover:-translate-y-1">
                          <h4 className="text-xl font-black text-[var(--text)]">{exp.role}</h4>
                          <p className="text-[var(--accent)] font-bold text-sm mt-1 mb-4 uppercase tracking-widest">{exp.company}</p>
                          <ul className="space-y-3">
                            {(exp.bullets || []).map((bullet, j) => (
                              <li key={j} className="flex items-start gap-3 text-sm text-[var(--muted)]">
                                <span className="text-[var(--accent)] mt-1.5 shadow-[0_0_5px_var(--accent)] block w-1.5 h-1.5 rounded-full shrink-0" />
                                <span>{bullet}</span>
                              </li>
                            ))}
                          </ul>
              ...
              {aboutPage.education?.enabled !== false && (
                <div>
                  <h3 className="text-3xl font-black text-[var(--text)] flex items-center gap-4 mb-10">
                    <span className="p-3 bg-[var(--surface-3)] rounded-xl text-[var(--accent-2)] shadow-[0_0_20px_rgba(59,130,246,0.15)]">
                      <GraduationCap size={28} />
                    </span>
                    {aboutPage.education.title}
                  </h3>
                  <div className="space-y-8 relative before:absolute before:inset-0 before:ml-[22px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-[var(--accent-2)] before:to-transparent">
                    {(aboutPage.education.items || []).map((edu, i) => (
                      <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active" data-aos="fade-up" data-aos-delay={i * 100}>
                        <div className="flex items-center justify-center w-12 h-12 rounded-full border-4 border-[var(--bg)] bg-[var(--accent-2)] text-white shadow-[0_0_15px_rgba(59,130,246,0.5)] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 transition-transform group-hover:scale-110">
                          <GraduationCap size={18} />
                        </div>
                        <div className="neon-card neon-glow-cyan p-6 w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] hover:-translate-y-1">
        ...
        <section className="py-16 border-y border-[var(--line)] bg-[var(--surface)] relative overflow-hidden">
          ...
          <div className="flex flex-wrap justify-center gap-4 max-w-5xl mx-auto px-4 relative z-10">
            {(aboutPage.techStack?.content || '').split('•').map(item => item.trim()).filter(Boolean).map((tech, i) => (
              <span 
                key={i} 
                className="flex items-center gap-2 px-6 py-3 rounded-xl border border-[var(--line)] bg-[var(--surface-2)] text-sm font-bold text-[var(--muted)] hover:text-[var(--text)] hover:border-[var(--accent)] hover:shadow-[0_0_15px_rgba(245,158,11,0.2)] transition-all cursor-default transform hover:-translate-y-1 group/tech" 
                data-aos="zoom-in" 
                data-aos-delay={(i % 12) * 50}
              >
                <span className="text-[var(--accent-2)] group-hover/tech:text-[var(--accent)] transition-colors text-lg">
                  {getTechIcon(tech)}
                </span>
                {tech}
              </span>
