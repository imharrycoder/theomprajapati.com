 ...
 * Parameterized by token storage key so the same logic serves
 * both the public frontend ('userToken') and admin ('adminToken').
 *
 * @param {string} tokenStorageKey — localStorage key for the auth token
 * @returns {Function} apiFetch(url, options) — configured fetch wrapper
 */
export function createApiFetch(tokenStorageKey) {
...
/**
 * Public frontend API client — uses 'userToken' from localStorage.
 */
export const apiFetch = createApiFetch('userToken');
